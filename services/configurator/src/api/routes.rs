...
pub fn config(cfg: &mut web::ServiceConfig) {
    cfg.service(
        web::scope(API_PREFIX)
            ...
            .route("/networks/{network_id}", web::get().to(networks::get_network_by_id))
            // Protected routes (require authentication)
            .route("/networks", web::post().to(networks::create_network))
            .route("/networks/{network_id}", web::put().to(networks::update_network))
            .route("/networks/{network_id}", web::delete().to(networks::delete_network)),
    );
}