...
use crate::domain::{DomainError, NetworkRepository};
...
impl<R: NetworkRepository + Send + Sync + Clone> NetworkApplicationService<R> {
    ...
    ) -> Result<crate::domain::Network, DomainError> {
        let handler = CreateNetworkHandler {
    ...
    pub async fn update_network(
        &self,
        command: UpdateNetworkCommand,
    ) -> Result<crate::domain::Network, DomainError> {
        let handler = UpdateNetworkHandler {
            repo: self.repo.clone(),
        };
        handler.execute(command).await
    }
    ...
    pub async fn delete_network(&self, command: DeleteNetworkCommand) -> Result<(), DomainError> {
        let handler = DeleteNetworkHandler {
            repo: self.repo.clone(),
        };
        handler.execute(command).await
    }
    ...
    ) -> Result<Vec<crate::domain::Network>, DomainError> {
        let handler = ListNetworksHandler {
            repo: self.repo.clone(),
        };
        handler.execute(query).await
    }
