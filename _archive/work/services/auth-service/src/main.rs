...
impl actix_web::ResponseError for AuthError {
    fn error_response(&self) -> HttpResponse {
        match self {
            ...
            AuthError::ConfigError(msg) => HttpResponse::InternalServerError().json(msg),
        }
    }
}
...
impl AuthService {
    pub fn new() -> Result<Self, AuthError> {
        ...
        let jwt_secret = std::env::var("JWT_SECRET").map_err(|_| {
            AuthError::ConfigError("JWT_SECRET environment variable is required".to_string())
        })?;

        let jwt_expiry_hours = std::env::var("JWT_EXPIRY_HOURS")
            .unwrap_or_else(|_| "24".to_string())
            .parse::<u64>()
            .map_err(|_| AuthError::ConfigError("Invalid JWT_EXPIRY_HOURS format".to_string()))?;
...
async fn main() -> std::io::Result<()> {
    ...
    info!("Server configuration: {}:{}", server_host, server_port);
