DROP MATERIALIZED VIEW IF EXISTS mv_charging_stations;

CREATE MATERIALIZED VIEW mv_charging_stations AS
SELECT
    -- Station info
    s.station_id::integer as station_id,
    s.network_id::integer as network_id,
    n.name::varchar as network_name,
    n.type::varchar as network_type,
    n.contact_email::varchar as contact_email,
    n.phone_number::varchar as phone_number,
    n.address::text as network_address,
    s.name::varchar as station_name,
    s.address::text as station_address,
    s.city::varchar as city,
    s.state::varchar as state,
    s.country::varchar as country,
    s.postal_code::varchar as postal_code,
    ST_X(s.location::geometry)::double precision as longitude,
    ST_Y(s.location::geometry)::double precision as latitude,
    CASE 
        WHEN s.tags IS NOT NULL THEN hstore_to_array(s.tags)::text[]
        ELSE NULL::text[]
    END as tags,
    s.status::varchar as station_status,
    s.is_operational::boolean as is_operational,
    ...
    )::boolean as has_available_connectors,
    ...
    (
        SELECT COALESCE(SUM(c.power_level_kw::double precision), 0)::double precision
        FROM connectors c
        WHERE c.station_id = s.station_id
    ...
    (
        SELECT ARRAY_AGG(DISTINCT ct.name)::text[]
        FROM connectors c
    ...
    (
        SELECT jsonb_agg(
            jsonb_build_object(
                ...
                'type_name', ct.name,
                'status', c.status,
                'power_level_kw', c.power_level_kw::double precision,
    ...
    (s.tags->'network')::jsonb as network,
    (s.tags->'opening_hours')::text as opening_hours,
    (s.tags->'capacity')::integer as capacity,
    (s.tags->'fee')::text as fee,
    (s.tags->'parking_fee')::text as parking_fee,
    (s.tags->'access')::text as access

FROM stations s
JOIN networks n ON s.network_id = n.network_id
WHERE s.location IS NOT NULL;

CREATE UNIQUE INDEX ON mv_charging_stations(station_id);
